...
export async function uploadImages(images:File[]) {
    ...
    const data=await Promise.all(
        images.map((file)=>
            supabase.storage.from("blogImages").upload(`${file.name}_${Date.now()}`,file)
        )
    )

    const urls=data.map((item)=>supabase.storage.from("blogImages").getPublicUrl(item.data?.path??"").data.publicUrl);
