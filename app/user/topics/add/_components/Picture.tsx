...
interface Props {
    ...
    savedImagesUrl?:BlogImage[];
    setSavedImagesUrl?:(topicImages:BlogImage[])=>void;
}
    ...
    return (
        <Card className={cn("p-3", props.className)}>
            ...
            {props.savedImagesUrl!! && props.savedImagesUrl.map((image,index)=>{
                return <PictureCard key={image.id} src={image.url} index={index} onDelete={(i)=>
                    props.setSavedImagesUrl!! && props.setSavedImagesUrl(props.savedImagesUrl!.filter(img=>img.id!==image.id))}/>
            })}
